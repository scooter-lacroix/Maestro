...
// Logs are stored in ~/.maestro/logs/<session-name>.log
...
func LogDir() string {
	...
	return filepath.Join(homeDir, ".maestro", "logs")
}
...
// SECURITY: Enforces strict limits to prevent session name attacks
...
func sanitizeName(name string) string {
	...
	// Enforce maximum length (50 chars)
	if len(sanitized) > 50 {
...
// SECURITY: Uses shquote for safe shell escaping to prevent command injection
func (s *Session) EnablePipePane() error {
	...
	// SECURITY: Use shquote to safely escape the log file path
	// This prevents command injection through malicious session names or paths
	escapedPath := shquote(logFile)

	// Enable pipe-pane: stream pane output to log file
	// The -o flag takes a shell command, so we must properly escape the path
	cmd := exec.Command("tmux", "pipe-pane", "-t", s.Name, "-o", "cat >> "+escapedPath)
...
// shquote safely quotes a string for shell use
// It wraps the string in single quotes and escapes any existing single quotes
// This prevents command injection when passing arguments to shell commands
func shquote(s string) string {
	// If string contains no single quotes or unsafe characters, return as-is
	// This keeps the output clean for simple cases
...
// ListAllSessions returns all Maestro tmux sessions
...
// DiscoverAllTmuxSessions returns all tmux sessions (including non-Maestro ones)
func DiscoverAllTmuxSessions() ([]*Session, error) {
	...
	for _, line := range lines {
		...
		// If it's an maestro-tui session, clean up the display name
