...
import (
	...
	"github.com/scooter-lacroix/maestro/tui/internal/mcppool"
)
...
func InitializeGlobalPool(ctx context.Context, config *UserConfig, sessions []*Instance) (*mcppool.Pool, error) {
	...
	// FIRST: Discover existing sockets from another maestro-tui instance
	// This allows multiple TUI instances to share the same pool
	discovered := pool.DiscoverExistingSockets()
	if discovered > 0 {
		log.Printf("[Pool] Reusing %d sockets from another maestro-tui instance", discovered)
	}
