...
func (h *Home) handleGlobalSearchSelection(result *GlobalSearchResult) tea.Cmd {
	// Check if session already exists in Maestro
	h.instancesMu.RLock()
	for _, inst := range h.instances {
...
// createSessionFromGlobalSearch creates a new Maestro session from global search result
...
func (h *Home) View() string {
	...
	if h.profile != "" && h.profile != session.DefaultProfile {
		profileStyle := lipgloss.NewStyle().
			...
			Bold(true)
		titleText = "Maestro " + profileStyle.Render("["+h.profile+"]")
	}
	...
	if h.updateInfo != nil && h.updateInfo.Available {
		...
		updateText := fmt.Sprintf(" ⬆ Update available: v%s → v%s (run: maestro-tui update) ",
...
func renderLoadingSplash(width, height int, frame int) string {
	...
	if width >= 40 && height >= 10 {
		...
		content.WriteString("\n")
		content.WriteString(titleStyle.Render("Maestro") + "\n")
		content.WriteString("\n")
		...
		// Minimal
		content.WriteString(greenStyle.Render("●") + " " + titleStyle.Render("Maestro") + "\n")
		content.WriteString(subtitleStyle.Render("Loading..."))
	}
