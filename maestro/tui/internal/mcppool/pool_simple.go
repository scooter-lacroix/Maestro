...
func (p *Pool) DiscoverExistingSockets() int {
	...
	for _, socketPath := range matches {
		// Extract MCP name from socket path: /tmp/maestro-mcp-{name}.sock
		base := filepath.Base(socketPath)
		if !strings.HasPrefix(base, "maestro-mcp-") || !strings.HasSuffix(base, ".sock") {
			continue
		}
		name := strings.TrimPrefix(base, "maestro-mcp-")
	...
	if discovered > 0 {
		log.Printf("[Pool] Discovered %d existing sockets from another maestro-tui instance", discovered)
	}
	return discovered
}
...
// RegisterExternalSocket registers an external socket owned by another maestro-tui instance.
