...
// validateMCPDef validates an MCP definition for security
// SECURITY: Ensures MCP commands are safe to execute
// Returns an error if the MCP definition is potentially dangerous
func validateMCPDef(name string, def session.MCPDef) error {
	// Check command is not empty
	if def.Command == "" {
		return fmt.Errorf("MCP '%s': command cannot be empty", name)
	}
	...
	for _, pattern := range dangerousPatterns {
		if strings.Contains(def.Command, pattern) {
			return fmt.Errorf("MCP '%s': command contains dangerous pattern '%s'", name, pattern)
		}
	}
	...
	for i, arg := range def.Args {
		for _, pattern := range dangerousPatterns {
			if strings.Contains(arg, pattern) {
				return fmt.Errorf("MCP '%s': arg[%d] contains dangerous pattern '%s'", name, i, pattern)
			}
		}
	}

	// Check env keys for dangerous patterns
	for key, val := range def.Env {
		for _, pattern := range dangerousPatterns {
			if strings.Contains(key, pattern) || strings.Contains(val, pattern) {
				return fmt.Errorf("MCP '%s': env contains dangerous pattern '%s'", name, pattern)
			}
		}
	}

	// SECURITY: Verify command exists in PATH (or is absolute path)
	// This prevents execution of arbitrary commands
	if !filepath.IsAbs(def.Command) {
		// Check if command exists in PATH
		_, err := exec.LookPath(def.Command)
		if err != nil {
			return fmt.Errorf("MCP '%s': command '%s' not found in PATH", name, def.Command)
		}
	} else {
		// Absolute path - verify it exists and is executable
		info, err := os.Stat(def.Command)
		if err != nil {
			return fmt.Errorf("MCP '%s': command path does not exist: %s", name, def.Command)
		}
		// Check it's a regular file (not directory or symlink to directory)
		if info.Mode()&0111 == 0 {
			return fmt.Errorf("MCP '%s': command is not executable: %s", name, def.Command)
		}
	}
...
func printMCPHelp() {
	fmt.Println("Usage: maestro-tui mcp <command> [options]")
	fmt.Println()
	...
	fmt.Println("  maestro-tui mcp list                        # List available MCPs")
	fmt.Println("  maestro-tui mcp attached                    # Show MCPs for current session")
	fmt.Println("  maestro-tui mcp attached my-project         # Show MCPs for specific session")
	fmt.Println("  maestro-tui mcp attach my-project exa       # Attach exa to my-project (local)")
	fmt.Println("  maestro-tui mcp attach my-project exa --global     # Attach globally")
	fmt.Println("  maestro-tui mcp detach my-project exa       # Detach exa from my-project")
}
...
func handleMCPList(args []string) {
	...
	fs.Usage = func() {
		fmt.Println("Usage: maestro-tui mcp list [options]")
		fmt.Println()
	...
	if len(mcps) == 0 {
		if *jsonOutput {
			...
			fmt.Println()
			fmt.Println("Define MCPs in ~/.maestro-tui/config.toml:")
			fmt.Println()
...
func handleMCPAttached(profile string, args []string) {
	...
	fs.Usage = func() {
		fmt.Println("Usage: maestro-tui mcp attached [session-id] [options]")
		fmt.Println()
...
func handleMCPAttach(profile string, args []string) {
	...
	fs.Usage = func() {
		fmt.Println("Usage: maestro-tui mcp attach <session-id> <mcp-name> [options]")
		fmt.Println()
		...
		fmt.Println()
		fmt.Println("Examples:")
		fmt.Println("  maestro-tui mcp attach my-project exa           # Attach locally")
		fmt.Println("  maestro-tui mcp attach my-project exa --global  # Attach globally")
		fmt.Println("  maestro-tui mcp attach my-project exa --restart # Attach and restart")
	}
	...
	if fs.NArg() < 2 {
		...
		if !*jsonOutput {
			fmt.Println("\nUsage: maestro-tui mcp attach <session-id> <mcp-name> [options]")
		}
		os.Exit(1)
	}
	...
	mcpDef, exists := availableMCPs[mcpName]
	if !exists {
	...
	// SECURITY: Validate MCP definition before attaching
	if err := validateMCPDef(mcpName, mcpDef); err != nil {
		out.Error(fmt.Sprintf("MCP validation failed: %v", err), ErrCodeInvalidOperation)
		os.Exit(2)
	}
...
func handleMCPDetach(profile string, args []string) {
	...
	fs.Usage = func() {
		fmt.Println("Usage: maestro-tui mcp detach <session-id> <mcp-name> [options]")
		fmt.Println()
		...
		fmt.Println()
		fmt.Println("Examples:")
		fmt.Println("  maestro-tui mcp detach my-project exa           # Detach from local")
		fmt.Println("  maestro-tui mcp detach my-project exa --global  # Detach from global")
		fmt.Println("  maestro-tui mcp detach my-project exa --restart # Detach and restart")
	}
	...
	if fs.NArg() < 2 {
		out.Error("session ID and MCP name are required", ErrCodeInvalidOperation)
		if !*jsonOutput {
			fmt.Println("\nUsage: maestro-tui mcp detach <session-id> <mcp-name> [options]")
		}
		os.Exit(1)
	}
