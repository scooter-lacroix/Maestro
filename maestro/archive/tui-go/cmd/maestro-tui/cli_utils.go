...
// GetCurrentSessionID detects the current maestro-tui session from tmux environment
// Returns session ID or empty string if not in an maestro-tui session
...
func ResolveSessionOrCurrent(identifier string, instances []*session.Instance) (*session.Instance, string, string) {
	if identifier == "" {
		...
		if currentID == "" {
			return nil, "no session specified and not inside an maestro-tui session", ErrCodeNotFound
		}
...
// validateProjectPath validates a project path for security
// SECURITY: Prevents path traversal attacks and symlink-based attacks
...
func validateProjectPath(path string) error {
	...
	// SECURITY: Resolve symlinks to prevent symlink-based attacks
	// An attacker could create a symlink to a sensitive directory
	resolvedPath, err := filepath.EvalSymlinks(absPath)
	if err != nil {
		return fmt.Errorf("failed to resolve symlinks: %w", err)
	}

	// Verify it's still a directory after resolving symlinks
	resolvedInfo, err := os.Stat(resolvedPath)
	if err != nil {
		return fmt.Errorf("resolved path does not exist: %s", resolvedPath)
	}
	if !resolvedInfo.IsDir() {
		return fmt.Errorf("resolved path is not a directory: %s", resolvedPath)
	}

	// Optional: Check if directory is writable (needed for session operations)
	// This prevents adding paths in read-only locations
	if err := os.WriteFile(filepath.Join(resolvedPath, ".maestro_test_write"), []byte("test"), 0644); err != nil {
		return fmt.Errorf("directory is not writable: %s", resolvedPath)
	}
	_ = os.Remove(filepath.Join(resolvedPath, ".maestro_test_write"))
