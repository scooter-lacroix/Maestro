...
import (
	...
	"github.com/scooter-lacroix/maestro/tui/internal/session"
	"github.com/scooter-lacroix/maestro/tui/internal/ui"
	"github.com/scooter-lacroix/maestro/tui/internal/update"
...
func printUpdateNotice() {
	...
	fmt.Fprintf(os.Stderr, "\n💡 Update available: v%s → v%s (run: maestro-tui update)\n",
...
func promptForUpdate() bool {
	...
	if response != "" && response != "y" && response != "yes" {
		fmt.Println("Skipped. Run 'maestro-tui update' later.")
		return false
	}
	...
	fmt.Println("Restart maestro-tui to use the new version.")
	return true
}
...
func initColorProfile() {
	...
	// MAESTRO_COLOR: truecolor, 256, 16, none
	if colorEnv := os.Getenv("MAESTRO_COLOR"); colorEnv != "" {
...
func main() {
	...
	if _, err := exec.LookPath("tmux"); err != nil {
		...
		fmt.Println("\nMaestro requires tmux. Install with:")
	...
	// Set up debug logging if MAESTRO_DEBUG is set
	// Logs go to ~/.maestro/debug.log to avoid interfering with TUI
	if os.Getenv("MAESTRO_DEBUG") != "" {
		if baseDir, err := session.GetMaestroDir(); err == nil {
			logPath := filepath.Join(baseDir, "debug.log")
			logFile, err := os.OpenFile(logPath, os.O_CREATE|os.O_WRONLY|os.O_TRUNC, 0644)
			if err == nil {
				log.SetOutput(logFile)
				log.SetFlags(log.Ltime | log.Lmicroseconds)
				log.Printf("=== Maestro Debug Log Started ===")
				defer logFile.Close()
			}
		}
	} else {
...
func handleAdd(profile string, args []string) {
	...
	fs.Usage = func() {
		fmt.Println("Usage: maestro-tui add [path] [options]")
		fmt.Println()
		fmt.Println("Add a new session to Maestro.")
		fmt.Println()
		...
		fmt.Println("  maestro-tui add                       # Use current directory")
		fmt.Println("  maestro-tui add /path/to/project")
		fmt.Println("  maestro-tui add -t \"My Project\" -g \"work\"")
		fmt.Println("  maestro-tui add -c claude .")
		fmt.Println("  maestro-tui -p work add               # Add to 'work' profile")
		fmt.Println("  maestro-tui add -t \"Sub-task\" --parent \"Main Project\"  # Create sub-session")
		fmt.Println("  maestro-tui add -t \"Research\" -c claude --mcp memory --mcp sequential-thinking /tmp/x")
	}
	...
	// SECURITY: Validate project path to prevent path traversal and symlink attacks
...
func handleList(profile string, args []string) {
	...
	fs.Usage = func() {
		fmt.Println("Usage: maestro-tui list [options]")
		fmt.Println()
		...
		fmt.Println()
		fmt.Println("Examples:")
		fmt.Println("  maestro-tui list                    # List from default profile")
		fmt.Println("  maestro-tui -p work list            # List from 'work' profile")
		fmt.Println("  maestro-tui list --all              # List from all profiles")
	}
...
func handleRemove(profile string, args []string) {
	...
	fs.Usage = func() {
		fmt.Println("Usage: maestro-tui remove <id|title>")
		fmt.Println()
		fmt.Println("Remove a session by ID or title.")
		fmt.Println()
		fmt.Println("Examples:")
		fmt.Println("  maestro-tui remove abc12345")
		fmt.Println("  maestro-tui remove \"My Project\"")
		fmt.Println("  maestro-tui -p work remove abc12345   # Remove from 'work' profile")
	}
		...
		if inst.ID == identifier || strings.HasPrefix(inst.ID, identifier) || inst.Title == identifier {
			...
			if inst.Exists() {
				if err := inst.Kill(); err != nil {
					...
					fmt.Println("Session removed from Maestro but may still be running in tmux")
				}
			}
		} else {
...
func handleStatus(profile string, args []string) {
	...
	fs.Usage = func() {
		fmt.Println("Usage: maestro-tui status [options]")
		fmt.Println()
		...
		fmt.Println()
		fmt.Println("Examples:")
		fmt.Println("  maestro-tui status              # Quick summary")
		fmt.Println("  maestro-tui status -v           # Detailed list")
		fmt.Println("  maestro-tui status -q           # Just waiting count")
		fmt.Println("  maestro-tui -p work status      # Status for 'work' profile")
	}
...
func handleProfile(args []string) {
	...
	case "create", "new":
		if len(args) < 2 {
			...
			fmt.Println("Usage: maestro-tui profile create <name>")
			os.Exit(1)
		}
	...
	case "delete", "rm":
		if len(args) < 2 {
			fmt.Println("Error: profile name is required")
			fmt.Println("Usage: maestro-tui profile delete <name>")
			os.Exit(1)
		}
	...
	default:
		...
		fmt.Println()
		fmt.Println("Usage: maestro-tui profile <command>")
		fmt.Println()
...
func handleProfileList() {
	...
	if len(profiles) == 0 {
		fmt.Println("No profiles found.")
		fmt.Println("Run 'maestro-tui' to create the default profile automatically.")
		return
	}
...
func handleProfileCreate(name string) {
	...
	fmt.Printf("  Use with: maestro-tui -p %s\n", name)
}
...
func handleUpdate(args []string) {
	...
	fs.Usage = func() {
		fmt.Println("Usage: maestro-tui update [options]")
		fmt.Println()
		...
		fmt.Println()
		fmt.Println("Examples:")
		fmt.Println("  maestro-tui update           # Check and install if available")
		fmt.Println("  maestro-tui update --check   # Only check, don't install")
	}
	...
	if *checkOnly {
		fmt.Println("\nRun 'maestro-tui update' to install.")
		return
	}
...
func printHelp() {
	...
	fmt.Println()
	fmt.Println("Usage: maestro-tui [-p profile] [command]")
	fmt.Println()
	...
	fmt.Println()
	fmt.Println("Examples:")
	fmt.Println("  maestro-tui                            # Start TUI with default profile")
	fmt.Println("  maestro-tui -p work                    # Start TUI with 'work' profile")
	fmt.Println("  maestro-tui add .                      # Add current directory")
	fmt.Println("  maestro-tui add -t \"My App\" -g dev .   # With title and group")
	fmt.Println("  maestro-tui session start my-project   # Start a session")
	fmt.Println("  maestro-tui session show               # Show current session (in tmux)")
	fmt.Println("  maestro-tui mcp list --json            # List MCPs as JSON")
	fmt.Println("  maestro-tui mcp attach my-app exa      # Attach MCP to session")
	fmt.Println("  maestro-tui group move my-app work     # Move session to group")
	fmt.Println()
	fmt.Println("Environment Variables:")
	fmt.Println("  MAESTRO_PROFILE    Default profile to use")
	fmt.Println("  MAESTRO_COLOR      Color mode: truecolor, 256, 16, none")
	fmt.Println()
...
func getLockFilePath(profile string) string {
	...
	return filepath.Join(homeDir, ".maestro", "profiles", profile, ".lock")
}
...
func acquireLock(profile string) error {
	...
	for attempt := 0; attempt < 2; attempt++ {
		...
		if readErr != nil {
			...
			os.Remove(lockPath)
			continue // Retry acquisition after removal
		}
