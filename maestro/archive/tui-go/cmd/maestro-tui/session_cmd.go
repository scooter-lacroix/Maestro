...
import (
	...
	"github.com/scooter-lacroix/maestro/tui/internal/profile"
	"github.com/scooter-lacroix/maestro/tui/internal/session"
	"github.com/scooter-lacroix/maestro/tui/internal/tmux"
)
...
func printSessionHelp() {
	fmt.Println("Usage: maestro-tui session <command> [options]")
	fmt.Println()
	...
	fmt.Println("  maestro-tui session start my-project")
	fmt.Println("  maestro-tui session stop abc123")
	fmt.Println("  maestro-tui session restart my-project")
	fmt.Println("  maestro-tui session fork my-project -t \"my-project-fork\"")
	fmt.Println("  maestro-tui session attach my-project")
	fmt.Println("  maestro-tui session show                  # Auto-detect current session")
	fmt.Println("  maestro-tui session show my-project --json")
	fmt.Println("  maestro-tui session set-parent sub-task main-project  # Make sub-task a sub-session")
	fmt.Println("  maestro-tui session unset-parent sub-task             # Remove sub-session link")
	fmt.Println("  maestro-tui session output my-project                 # Get last response from session")
	fmt.Println("  maestro-tui session output my-project --json          # Get response as JSON")
	fmt.Println()
	...
	fmt.Println("  maestro-tui session set my-project title \"New Title\"")
	fmt.Println("  maestro-tui session set my-project claude-session-id \"abc123-def456\"")
	fmt.Println("  maestro-tui session set my-project tool claude")
}
...
func handleSessionStart(profile string, args []string) {
	...
	fs.Usage = func() {
		fmt.Println("Usage: maestro-tui session start <id|title> [options]")
		fmt.Println()
		...
		fmt.Println("Examples:")
		fmt.Println("  maestro-tui session start my-project")
		fmt.Println("  maestro-tui session start my-project --message \"Research MCP patterns\"")
		fmt.Println("  maestro-tui session start my-project -m \"Explain this codebase\"")
	}
...
func handleSessionStop(profile string, args []string) {
	...
	fs.Usage = func() {
		fmt.Println("Usage: maestro-tui session stop <id|title> [options]")
		fmt.Println()
...
func handleSessionRestart(profile string, args []string) {
	...
	fs.Usage = func() {
		fmt.Println("Usage: maestro-tui session restart <id|title> [options]")
		fmt.Println()
...
func handleSessionFork(profile string, args []string) {
	...
	fs.Usage = func() {
		fmt.Println("Usage: maestro-tui session fork <id|title> [options]")
		fmt.Println()
		...
		fmt.Println()
		fmt.Println("Examples:")
		fmt.Println("  maestro-tui session fork my-project")
		fmt.Println("  maestro-tui session fork my-project -t \"my-fork\"")
		fmt.Println("  maestro-tui session fork my-project -t \"my-fork\" -g \"experiments\"")
	}
...
func handleSessionAttach(profile string, args []string) {
	...
	fs.Usage = func() {
		fmt.Println("Usage: maestro-tui session attach <id|title>")
		fmt.Println()
...
func handleSessionShow(profile string, args []string) {
	...
	fs.Usage = func() {
		fmt.Println("Usage: maestro-tui session show [id|title] [options]")
		fmt.Println()
...
func handleSessionSet(profile string, args []string) {
	...
	fs.Usage = func() {
		fmt.Println("Usage: maestro-tui session set <id|title> <field> <value> [options]")
		fmt.Println()
		...
		fmt.Println("  maestro-tui session set my-project title \"New Title\"")
		fmt.Println("  maestro-tui session set my-project claude-session-id \"abc123-def456\"")
		fmt.Println("  maestro-tui session set my-project path /new/path/to/project")
	}
...
func findSessionByTmux(instances []*session.Instance) *session.Instance {
	...
	// Parse maestro-tui session name: maestro_<title>_<id>
	if strings.HasPrefix(sessionName, "maestro_") {
		// Extract title (everything between maestro_ and the last _id)
		withoutPrefix := strings.TrimPrefix(sessionName, "maestro_")
		...
		if lastUnderscore > 0 {
			...
			// For maestro sessions, we have the title - don't fall back to path matching
			// as that could match a different session with same path in another profile
			return nil
		}
	}

	// Try to find by path (only for non-maestro tmux sessions)
	for _, inst := range instances {
...
func showTmuxSessionInfo(out *CLIOutput, jsonOutput bool) {
	...
	var sb strings.Builder
	sb.WriteString("⚠ Session not registered in maestro-tui\n")
	sb.WriteString(fmt.Sprintf("Tmux:    %s\n", sessionName))
	...
	sb.WriteString(fmt.Sprintf("  maestro-tui add -t \"%s\" -g <group> -c claude %s\n", title, currentPath))
...
func handleSessionSetParent(profile string, args []string) {
	...
	fs.Usage = func() {
		fmt.Println("Usage: maestro-tui session set-parent <session> <parent>")
		fmt.Println()
...
func handleSessionUnsetParent(profile string, args []string) {
	...
	fs.Usage = func() {
		fmt.Println("Usage: maestro-tui session unset-parent <session>")
		fmt.Println()
...
func handleSessionSend(profile string, args []string) {
	...
	if len(remaining) < 2 {
		out.Error("usage: maestro-tui session send <id> <message>", ErrCodeInvalidOperation)
		os.Exit(1)
	}
...
// SECURITY: Uses context with timeout to prevent infinite hangs
func waitForAgentReady(tmuxSess *tmux.Session, tool string) error {
	...
	// This prevents the function from hanging indefinitely if the agent never becomes ready
	...
	for attempt := 0; attempt < maxAttempts; attempt++ {
		// Check if context is cancelled (timeout reached)
		select {
		case <-ctx.Done():
			return fmt.Errorf("agent not ready after 60 seconds (context timeout)")
		default:
			// Continue
		}
...
func handleSessionOutput(profile string, args []string) {
	...
	fs.Usage = func() {
		fmt.Println("Usage: maestro-tui session output [id|title] [options]")
		fmt.Println()
...
func handleSessionCurrent(profileArg string, args []string) {
	...
	fs.Usage = func() {
		fmt.Println("Usage: maestro-tui session current [options]")
		fmt.Println()
	...
	if inst == nil {
		out.Error("current tmux session is not an maestro-tui session\nHint: Run 'maestro-tui list' to see available sessions", ErrCodeNotFound)
		os.Exit(1)
	}
