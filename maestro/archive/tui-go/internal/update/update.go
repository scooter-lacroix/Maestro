...
const (
	...
	GitHubRepo = "scooter-lacroix/maestro"
...
func getCacheDir() (string, error) {
	...
	return filepath.Join(home, ".maestro-tui"), nil
}
...
func getAssetURL(release *Release) string {
	...
	// Construct expected asset name: maestro-tui_X.Y.Z_os_arch.tar.gz
	version := strings.TrimPrefix(release.TagName, "v")
	expectedName := fmt.Sprintf("maestro-tui_%s_%s_%s.tar.gz", version, goos, goarch)
...
func PerformUpdate(downloadURL string) error {
	...
	tmpFile, err := os.CreateTemp("", "maestro-tui-update-*.tar.gz")
	if err != nil {
...
// extractBinaryFromTarGz extracts the maestro-tui binary from a .tar.gz file
func extractBinaryFromTarGz(tarPath string) ([]byte, error) {
	...
	for {
		...
		// Look for the maestro-tui binary
		if header.Typeflag == tar.TypeReg && header.Name == "maestro-tui" {
	...
	return nil, fmt.Errorf("maestro-tui binary not found in archive")
}
