#!/usr/bin/env python3
"""
Maestro Pre-Tool-Use Hook: TLDR Context Injection

Injects TLDR code analysis context into prompts based on intent analysis.
Adds relevant code structure context to improve Task execution using
Maestro TLDR commands (/maestro:tree, /maestro:structure, etc.)
"""
...
def get_relevant_code_context(
    ...
    """
    Get relevant code context using TLDR analysis.
    ...
    # Use the TLDR context extractor
    try:
        context_func = _optional_attr("maestro.tldr.context", "get_context_for_prompt")
        ctx = context_func(working_dir, prompt, max_files=3) if callable(context_func) else ""
        if ctx and ctx.strip() != "# No specific files identified":
            context_parts.append(ctx)
    except Exception:
        pass

    # If TLDR context is not available, do basic analysis
    analyzer_cls = _optional_attr("maestro.tldr.analyzer", "TLRDAnalyzer")
    if not context_parts and analyzer_cls is not None:
        try:
            ...
            for file_path in files[:3]:
                result = analyzer.analyze_file(file_path, layers=[1, 2])
                if result and hasattr(result, 'ast_analysis') and result.ast_analysis:
                    ast_analyzer_cls = _optional_attr("maestro.tldr.ast", "ASTAnalyzer")
                    if ast_analyzer_cls is not None:
                        ast_analyzer = ast_analyzer_cls()
                        file_ctx = ast_analyzer.to_llm_string(result.ast_analysis)
...
def tldr_context_hook(input_data: dict) -> dict:
    """
    Pre-tool-use hook that injects TLDR context for Task operations.

    This hook analyzes the prompt intent and injects relevant:
    - Code structure from TLDR analysis
    ...
    try:
        ...
        # Recall relevant memories from TLDR analysis
        ...
        if tool_name == "Edit" and context_info["context_injected"]:
            ...
            if code_context:
                context_prefix += f"<!-- TLDR Code Context -->\n{code_context}\n\n"
        ...
        input_data["maestro_tldr_context"] = context_info
...
def main() -> None:
    ...
    result = tldr_context_hook(input_data)
