...
pub fn render_dashboard(f: &mut Frame, area: Rect, state: &ConductorState) {
    ...
    let git_info = match &state.git_info {
        Some(gi) => format!(
            "{} ({})",
            gi.branch.as_deref().unwrap_or("unknown"),
            if gi.is_dirty { "dirty" } else { "clean" }
        ),
    ...
    let active_subagents = state
        .subagents
        .iter()
        .filter(|s| s.status == crate::conductor::model::SubagentStatus::Running)
        .count();
    ...
    let rate_limit_info = if let Some(rl) = &state.rate_limit {
        if rl.limited_at.is_some() {
            format!(
                "LIMITED (until {})",
                rl.backoff_until
                    .map(|b| b.format("%H:%M:%S").to_string())
                    .unwrap_or_else(|| "N/A".to_string())
            )
        } else {
