...
mod tests {
    ...
    fn test_master_track_detection() {
        ...
        assert!(master_track.is_some(), "Master track should be detected by ID pattern 'master'");
    }

    #[test]
    fn test_master_track_detection_via_metadata() {
        use leindex_core::orchestrate::model::{Track, TrackMetadata, TrackType, TrackStatus};
        ...
        assert!(master_track.is_some(), "Master track should be detected by metadata type");
    }
    ...
    fn test_external_session_discovery() {
        ...
        let ext_track = items.iter().find(|i| match i {
            SelectableItem::Track { id, is_external, .. } => id == "ext-track" && *is_external,
            _ => false,
        });
    ...
    fn test_process_iteration_log_suppression() {
        ...
        assert!(pane.iteration_output.is_empty(), "Output should be suppressed during initial read");
        assert_eq!(pane.state.iteration_logs.len(), 1, "History should still be updated");
        
        // Live update (suppress_output = false)
        pane.process_iteration_log(log, false);
        assert!(!pane.iteration_output.is_empty(), "Output should NOT be suppressed during live update");
    }
