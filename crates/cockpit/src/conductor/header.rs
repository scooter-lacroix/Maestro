...
pub fn render_header(frame: &mut Frame, area: Rect, state: &ConductorState) {
    ...
    let status_color = match state.status {
        super::model::ConductorStatus::Running | super::model::ConductorStatus::Executing => theme.status_success,
        super::model::ConductorStatus::Pausing | super::model::ConductorStatus::Paused => theme.status_warning,
    ...
    let agent_info = if let Some(agent) = &state.active_agent {
        format!(" @ {} ({})", agent.tool, agent.model.as_deref().unwrap_or("default"))
    } else {
        "".to_string()
    };
    ...
    let content = Line::from(vec![
        Span::styled(status_text, Style::default().bg(status_color).fg(Color::Black).bold()),
        Span::styled(format!(" Track: {} ", track_text), Style::default().fg(theme.accent_secondary).bold()),
        Span::styled(format!("> Task: {} ", task_text), Style::default().fg(theme.fg_primary)),
