//! Capabilities tab rendering for Cockpit TUI
//!
//! This tab displays Phase 3 capabilities:
...
fn render_section_tabs(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let cron_tab = Paragraph::new(format!(" ⏰ Cron Jobs ({}) ", cron_count))
    ...
    let mcp_tab = Paragraph::new(" 🔌 MCP Servers ")
        .alignment(Alignment::Center)
        .block(
            Block::default()
                .borders(Borders::ALL)
    ...
    let sandbox_tab = Paragraph::new(" 🔒 Sandbox ")
        .alignment(Alignment::Center)
        .block(
            Block::default()
                .borders(Borders::ALL)
...
fn render_cron_jobs(frame: &mut Frame, app: &App, area: Rect, theme: crate::theme::Theme) {
    ...
    let rows: Vec<Row> = app.cron_jobs.iter().map(|job| {
...
fn render_mcp_servers(frame: &mut Frame, app: &App, area: Rect, theme: crate::theme::Theme) {
    ...
    let servers: Vec<ListItem> = if registered.is_empty() {
        ...
        registered.iter().map(|name| {
            let is_connected = connected.contains(name);
            let status = if is_connected { "●" } else { "○" };
            let status_text = if is_connected { "Ready" } else { "Not Connected" };
            let style = if is_connected {
                ratatui::style::Style::default().fg(theme.success)
            } else {
                ratatui::style::Style::default().fg(theme.muted)
            };
            ListItem::new(format!(" {} {:<20} [{}]", status, name, status_text))
                .style(style)
        }).collect()
    };
...
fn render_sandbox(frame: &mut Frame, app: &App, area: Rect, theme: crate::theme::Theme) {
    ...
    let network_str = if policy.allow_network { "Enabled" } else { "Disabled" };
    let network_color = if policy.allow_network { theme.success } else { theme.warning };

    let policy_lines = vec![
        Line::from(vec![
            Span::styled("Autonomy Level: ", ratatui::style::Style::default().fg(theme.muted)),
            Span::styled(autonomy_str, ratatui::style::Style::default().fg(autonomy_color).bold()),
        ]),
        Line::from(vec![
            Span::styled("Memory Limit: ", ratatui::style::Style::default().fg(theme.muted)),
            Span::raw(memory_str),
        ]),
        Line::from(vec![
            Span::styled("CPU Shares: ", ratatui::style::Style::default().fg(theme.muted)),
            Span::raw(format!("{}", policy.max_cpu_shares)),
        ]),
        Line::from(vec![
            Span::styled("Network: ", ratatui::style::Style::default().fg(theme.muted)),
            Span::styled(network_str, ratatui::style::Style::default().fg(network_color)),
        ]),
        Line::from(vec![
            Span::styled("Timeout: ", ratatui::style::Style::default().fg(theme.muted)),
            Span::raw(format!("{:?}", policy.max_execution_time)),
        ]),
    ];

    let policy_para = Paragraph::new(policy_lines).block(Block::default().padding(ratatui::widgets::Padding::new(1, 1, 1, 1)));
    ...
    let runtime_items: Vec<ListItem> = runtimes.iter().map(|r| {
        match *r {
            "native" => ListItem::new(" ● native      Native process execution (trusted code only)")
                .style(ratatui::style::Style::default().fg(theme.success)),
            "wasm" => ListItem::new(" ○ wasm        WASM sandbox (experimental)")
                .style(ratatui::style::Style::default().fg(theme.muted)),
            "docker" => ListItem::new(" ○ docker      Docker container isolation (requires Docker)")
                .style(ratatui::style::Style::default().fg(theme.muted)),
