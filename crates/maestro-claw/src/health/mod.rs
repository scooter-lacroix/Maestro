//! Global health registry for MaestroClaw daemon components
...
{
    let mut map = registry().components.lock().unwrap();
    let now = now_rfc3339();
    let entry = map
...
pub fn snapshot() -> HealthSnapshot {
    let components = registry().components.lock().unwrap().clone();
    HealthSnapshot {
        pid: std::process::id(),
...
pub fn snapshot_json() -> serde_json::Value {
    serde_json::to_value(snapshot()).unwrap_or_else(|_| {
        serde_json::json!({ "status": "error", "message": "failed to serialize health snapshot" })
    })
}
...
mod tests {
    ...
    #[test]
    fn mark_ok_then_error_then_ok() {
        let c = unique_component("health-cycle");
        mark_component_ok(&c);
        let s = snapshot();
        assert_eq!(s.components[&c].status, "ok");

        mark_component_error(&c, "fail");
        let s = snapshot();
        assert_eq!(s.components[&c].status, "error");

        mark_component_ok(&c);
        let s = snapshot();
        assert_eq!(s.components[&c].status, "ok");
        assert!(s.components[&c].last_error.is_none());
    }

    #[test]
    fn bump_restart_increments() {
        let c = unique_component("health-restart");
        bump_component_restart(&c);
        bump_component_restart(&c);
        assert_eq!(snapshot().components[&c].restart_count, 2);
    }
}
