/**
 * TrackLens UI - Mermaid Block Component
 *
 * Renders mermaid diagrams with zoom/pan controls.
 *
...
export const MermaidBlock: React.FC<{ block: Block }> = ({ block }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const padding = 8;
      const base: ViewBox = { x: bbox.x - padding, y: bbox.y - padding, width: bbox.width + padding * 2, height: bbox.height + padding * 2 };
      baseViewBoxRef.current = base;
      applyView(svgEl, base, 1.0, { x: 0, y: 0 });
    } catch { /* Ignore */ }
  }, [svg, showSource]);

  const handleZoomIn = useCallback(() => updateZoom(Math.min(zoomLevelRef.current + ZOOM_STEP, MAX_ZOOM)), [updateZoom]);
  const handleZoomOut = useCallback(() => updateZoom(Math.max(zoomLevelRef.current - ZOOM_STEP, MIN_ZOOM)), [updateZoom]);
  const handleFitToScreen = useCallback(() => { panOffsetRef.current = { x: 0, y: 0 }; updateZoom(1.0); }, [updateZoom]);
  ...
  if (error) {
    return (
      <div className="my-5 rounded-lg border border-destructive/30 bg-destructive/5 overflow-hidden">
        ...
        <pre className="p-3 text-xs text-muted-foreground bg-muted/30 border-t border-border/30 overflow-x-auto"><code>{block.content}</code></pre>
      </div>
    );
  }
  ...
  return (
    <div className="my-5 group relative" data-block-id={block.id}>
      <div className="absolute top-2 right-2 flex flex-col gap-1 items-end opacity-0 group-hover:opacity-100 transition-opacity z-10">
        <button onClick={() => setShowSource(!showSource)} className="p-1.5 rounded-md bg-muted/80 hover:bg-muted text-muted-foreground hover:text-foreground" title={showSource ? 'Show diagram' : 'Show source'}>
          {showSource ? (
        ...
        {!showSource && svg && (
          <div className="flex flex-col gap-0.5 bg-muted/80 rounded-md p-0.5">
            <button ref={zoomInBtnRef} onClick={handleZoomIn} className="p-1 rounded hover:bg-muted" title="Zoom in">
              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 5v14M5 12h14" /></svg>
            </button>
            <button onClick={handleFitToScreen} className="p-1 rounded hover:bg-muted" title="Fit to screen">
            ...
            <button ref={zoomOutBtnRef} onClick={handleZoomOut} className="p-1 rounded hover:bg-muted" title="Zoom out">
            ...
            <span ref={zoomDisplayRef} hidden className="text-[10px] text-center text-muted-foreground tabular-nums leading-tight" />
          </div>
        )}
      </div>
      ...
      {!showSource && svg && (
        <div ref={containerRef} className="absolute inset-0 rounded-lg bg-muted/30 border border-border/30 p-4 overflow-hidden flex justify-center select-none" dangerouslySetInnerHTML={{ __html: svg }} />
      )}
    </div>
  );
};
