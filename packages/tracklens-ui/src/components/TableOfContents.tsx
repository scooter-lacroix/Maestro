/**
 * TrackLens UI - Table of Contents Component
 *
 * Hierarchical table of contents with annotation counts and navigation.
 *
    ...
    <li className="list-none">
      <div className="flex items-start group">
        ...
        <button
          ...
          className={`flex-1 text-left text-xs py-0.5 px-1.5 rounded transition-colors ${indent} ${hasChildren ? '' : 'ml-5'} ${
            isActive
        ...
        >
          <span className="flex items-center justify-between gap-2">
            ...
            {item.annotationCount > 0 && (
              <span className="flex-shrink-0 bg-accent text-accent-foreground rounded-full w-5 h-5 flex items-center justify-center text-[10px] font-medium">
      ...
      {hasChildren && isExpanded && (
        <ul className="mt-0.5 space-y-0.5">
          {item.children.map((child) => (
            <TocItemWithState key={child.id} item={child} activeId={activeId} onNavigate={onNavigate} />
          ))}
        </ul>
      )}
    </li>
...
function TocItemWithState({
  ...
  activeId,
  onNavigate,
}: { item: TocItem; activeId: string | null; onNavigate: (blockId: string) => void }) {
...
export function TableOfContents({
  ...
  const annotationCounts = useMemo(() => getAnnotationCountBySection(blocks, annotations), [blocks, annotations]);
  const tocItems = useMemo(() => buildTocHierarchy(blocks, annotationCounts), [blocks, annotationCounts]);
  ...
  return (
    <nav className={className || "bg-card/50 backdrop-blur-sm border-r border-border overflow-y-auto"} style={style}>
      <div className="px-3 py-2">
        {linkedDocFilepath && (
          <div className="mb-2 pb-1.5 border-b border-border/50">
            <div className="flex items-center justify-between">
              ...
              {onLinkedDocBack && (
                <button onClick={onLinkedDocBack} className="flex items-center gap-0.5 text-[10px] font-medium text-primary hover:text-primary/80">
        ...
        <ul className="space-y-0.5">
          {tocItems.map((item) => (
            <TocItemWithState key={item.id} item={item} activeId={activeId} onNavigate={handleNavigate} />
          ))}
        </ul>
      </div>
    </nav>
