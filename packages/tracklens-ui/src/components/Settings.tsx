/**
 * TrackLens UI - Settings Component
 *
 * Settings modal with General/Display/Saving tabs.
 * Removed: TaterSprite mascot, sharing features.
 *
...
export const Settings: React.FC<SettingsProps> = ({ onIdentityChange, origin, mode = 'plan', onUIPreferencesChange }) => {
  ...
  const { agents: availableAgents, validateAgent, getAgentWarning: getAgentWarningFn } = useAgents(origin ?? null);
  ...
  useEffect(() => {
    if (showDialog) {
      ...
      if (origin === 'opencode') {
        setAgentWarning(getAgentWarningFn());
      }
    }
  }, [showDialog, origin, getAgentWarningFn]);
  ...
  }, [obsidian.enabled, detectedVaults.length, vaultsLoading]);
